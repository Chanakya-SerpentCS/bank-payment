...
class IppayController(http.Controller):
    ...
    # _notireturn request.render("ippay_payment.failed", kwargs)

    @http.route(['/payment/ippay/s2s/create_json_3ds'],
                type='json', auth='public', csrf=False)
    def ippay_s2s_create_json_3ds(self,
                                  verify_validity=False, **kwargs):
